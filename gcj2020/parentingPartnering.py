...
cover the activity from 18:00 to 20:00 and Jamie to cover the other two. Notice that 
the first two activities overlap in the time between 19:00 and 20:00, so it is impossible 
...
Then, N more lines follow. The i-th of these lines (counting starting from 1) contains two integers 
Si and Ei. The i-th activity starts exactly Si minutes after midnight and ends exactly Ei minutes after midnight.


Output
For each test case, output one line containing Case #x: y, where x is the test case number 
(starting from 1) and y is IMPOSSIBLE if there is no valid schedule according to the above rules, 
or a string of exactly N characters otherwise. The i-th character in y must be C if the i-th activity 
is assigned to Cameron in your proposed schedule, and J if it is assigned to Jamie.
...
# traverse dfs
# so assign the latest time and go on assigning tasks from the sorted list
# like here, [[1, 100], [2, 5], [99, 150], [100, 301], [150, 250]]
# assign the first to J always as j and c are symmetric, ie if it is gonna work with j first then it will work with c first
# so go with j first and then at a choice, dfs
# so next element
# if can go to only j or only c then great, if neither then stop, if both then explore both.
#  
 

def schedule(l):
    #l is a list of timings that is sorted
    c=0
    j=0
    lfinal=['' for i in range(len(l))]

    # go on assigning to c or to j 
    # if both are assignable then assign to c
    # if none are assignable then return impossible
...
# l=sorted([[99, 150, 0],[1, 100, 1],[100, 301, 2], [2, 5, 3], [150, 250, 4]])
...
for case in range(1,t+1):
    ...
    print("Case #"+ str(case) +": "+schedule(sorted(l)))
